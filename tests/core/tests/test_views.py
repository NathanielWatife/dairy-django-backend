...
class TestCowInventoryViewSet:
    ...
    def test_list_cow_inventory(self, user_type, expected_status):
        url = reverse("core:cow-inventory-list")
        response = self.client.get(url, HTTP_AUTHORIZATION=f"Token {self.tokens[user_type]}")
        assert response.status_code == expected_status
    ...
    def test_create_cow_inventory(self, user_type, expected_status):
        ...
        response = self.client.post(url, data, HTTP_AUTHORIZATION=f"Token {self.tokens[user_type]}")
        assert response.status_code == expected_status
    ...
    def test_delete_cow_inventory(self, user_type, expected_status):
        ...
        cow_inventory_url = reverse("core:cow-inventory-list")
        cow_inventory_response = self.client.get(cow_inventory_url, HTTP_AUTHORIZATION=f"Token {self.tokens[user_type]}")
        cow_inventory_id = cow_inventory_response.data.get("id", None)

        if cow_inventory_id is not None:
            delete_url = reverse("core:cow-inventory-detail", kwargs={"pk": cow_inventory_id})
            response = self.client.delete(delete_url, HTTP_AUTHORIZATION=f"Token {self.tokens[user_type]}")
            assert response.status_code == expected_status